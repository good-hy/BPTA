...
int Basic_information::FindLatestWorkerNew_Greedy(int taskid,
                                                  ...
                                                  vector<int> CT_Worker[], double MaxDistanceTask[], double *current_task_NeedTime,
                                                  vector<CURRENT_TASK_GROUP> &current_taskGroup,
                                                  vector<CURRENT_WORKERS_GROUP> &current_workerGroup,
                                                  vector<int> current_poi[], double nowtime)
{
        ...
        if (current_taskGroup[taskid].task.Minscore <= current_workerGroup[workerid].worker.score)
        {
            ...
            if (current_workerGroup[workerid].worker.range >= dist)
            {
                ...
                if (current_taskGroup[taskid].task.Reward - (2 * dist * c) > 0)
                {
                    ...
                    if (CurrentTask_Satisfy(current_taskGroup, workerid, taskid, current_Group_workerAD,
                                            current_detour_distance, current_Group_worker_subTrajectoryDis,
                                            CT_Worker, current_poi, MaxDistanceTask, current_task_NeedTime, nowtime))
    ...
    for (int i = 0; i < Number_Worker; i++)
    {
        ...
        for (int j = 0; j < CT_Worker[i].size(); j++)
        {
            ...
            if (it != PT[taskid].end())
            {
                ...
                double s = ((PT[taskid][index].second - global_tasks[taskid].Minscore + 1) /
                            (PT[taskid][ttt].second - global_tasks[taskid].Minscore + 1));
    ...
    for (int i = 0; i < Number_Worker; i++)
    {
        ...
        for (int j = 0; j < CT_Worker[i].size(); j++)
        {
            ...
            if (it != PT[taskid].end())
            {
                ...
                s *= global_MaxDistanceTask[taskid] / ((global_tasks[taskid].Deadline - global_tasks[taskid].startTime) *
                                                           speed -
                                                       global_detour_distance[taskid][workerID]);
    ...
    for (int i = 0; i < Number_Worker; i++)
    {
        ...
        if (CT_Worker[i].size() != 0)
        {
            ...
            for (int j = 0; j < CT_Worker[i].size(); j++)
            {
                ...
                {
                    int index = distance(PW[workerID].begin(), it);
                    double s = PW[workerID][index].second / PW[workerID][ttt].second;
...
bool Basic_information::IfReplace(int workerid, int taskid, int assignedtaskid, vector<double> &AD,
                                  ...
                                  double beforesumDis, vector<CURRENT_TASK_GROUP> &current_taskGroup, double nowtime)
{
...
void Basic_information::Prodece_Task_Reward_Minscore_Deadline(vector<TASK> &task)
{
    ...
    uniform_real_distribution<double> u2(8, 10);
    ...
    for (int i = 0; i < Number_Worker; i++)
    {
        ...
        else
        {
            ...
            for (int j = 0; j < worker[i].trajectory.size() - 1; j++)
            {
                ...
                double distance = GetDistance(worker[i].trajectory[j].Y, worker[i].trajectory[j].X,
                                              worker[i].trajectory[j + 1].Y, worker[i].trajectory[j + 1].X);
                sum = sum + distance;
...
void Basic_information::updata_current_Info(vector<CURRENT_TASK_GROUP> &current_taskGroup, vector<CURRENT_WORKERS_GROUP> &current_workerGroup,
                                            vector<double> &current_Group_workerAD, vector<vector<double>> &current_Group_worker_subTrajectoryDis,
                                            vector<double> &current_Group_workerSumdis)
{
    ...
    for (vector<CURRENT_WORKERS_GROUP>::iterator it_curret_Worker = current_workerGroup.begin();
         it_curret_Worker != current_workerGroup.end(); ++it_curret_Worker)
    {
...

{
    ...
    for (int i = 0; i < tasks.size(); i++)
    {
...

        {
            ...
            groupWork_according_TaskGroup(workers, Sumdis, current_Group_worker, current_Group_workerSumdis, current_window_endTime,
                                          current_workID, current_Group_workerAD, global_Worker_subTrajectoryDis,
                                          current_Group_worker_subTrajectoryDis);

            determine_Window_Task_Timeout(current_taskGroup, current_window_endTime);

            match_WorkerTask_Greedy(current_taskGroup, current_Group_worker, current_window_endTime, current_Group_workerAD,
                                    current_Group_worker_subTrajectoryDis);
            ...
            updata_current_Info(current_taskGroup, current_Group_worker, current_Group_workerAD, current_Group_worker_subTrajectoryDis,
                                current_Group_workerSumdis);
        }
    }
                                                      ...
                                                      vector<vector<double>> &current_Group_worker_subTrajectoryDis)
{
    ...
    for (auto it = (workers.begin() + current_workID); it != workers.end();)
    {
        ...
        if (it->startTime >= nowTime)
        {
            ...
            Initialize_group(current_Group_workerAD, current_Group_workerSumdis, current_Group_worker_subTrajectoryDis, current_workID, nowTime,
                             current_workerGroup);
                                                ...
                                                vector<vector<double>> &current_Group_worker_subTrajectoryDis)
{
    ...
    for (int i = 0; i < current_Number_Task; i++)
    {
        ...
        if (current_Group_worker_AW.size() != 0)
        {
            ...
            workerid = FindLatestWorkerNew_Greedy(i, current_Group_worker_AW, current_Group_workerAD, current_detour_distance,
                                                  current_Group_worker_subTrajectoryDis, current_CT_Worker, current_MaxDistanceTask,
                                                  &current_task_NeedTime, current_taskGroup, current_workerGroup, current_poi, current_window_endTime);
            ...
            if (workerid != -1)
            {
                ...
                UpdateTaskDeadline(current_taskGroup, workerid, i, current_detour_distance, current_CT_Worker, current_poi,
                                   current_MaxDistanceTask, current_task_NeedTime);
...
void Basic_information::printf_Satisfaction_Results_num(string alg_name, double run_time, int test_range, int test_task, int test_worker)
{
    ...
    if (Satisfaction_sign)
    {
        ...
        printf_Results_to_txt_num(task_satis_results, worker_satis_results, "平均", alg_name, run_time, test_range, test_task, test_worker);
    }
    else
    {
        ...
        printf_Results_to_txt_num(task_satis_results, worker_satis_results, "求和", alg_name, run_time, test_range, test_task, test_worker);
    }
}
...
void Basic_information::printf_Results_to_txt(double task_satis_results, double worker_satis_results, string sati_name, string alg_name, double run_time)
{
    ...
    out << alg_name;
    out << " " << Number_Task << " " << Number_Worker << " " << Capacity << " " << c << " " << speed
        << " " << Wmax << " " << Tmax;
    out << " " << task_satis_results;
    out << " " << worker_satis_results;
    out << " " << sumtasks;
    out << " " << workers;
    out << " " << run_time;
    out << "\n";
...
void Basic_information::printf_Results_to_txt_num(double task_satis_results, double worker_satis_results, string sati_name,
                                                  string alg_name, double run_time, int test_range, int test_task, int test_worker)
{
    ...
    out << " " << workers;
    out << " " << run_time;
    out << " " << test_range << " " << test_task << " " << test_worker;
    out << "\n";
    out.close();
...
void Basic_information::match_WorkerTask_workerBatch(vector<CURRENT_TASK_GROUP> &current_taskGroup, vector<CURRENT_WORKERS_GROUP> &current_workerGroup,
                                                     double current_window_endTime, vector<double> &current_Group_workerAD,
                                                     vector<vector<double>> &current_Group_worker_subTrajectoryDis)
{
    int current_Number_Worker = current_workerGroup.size();
...
void Basic_information::iterator_Match_WorkBatch(vector<CURRENT_WORKERS_GROUP> &current_workerGroup, vector<CURRENT_TASK_GROUP> &current_taskGroup,
                                                 vector<vector<pair<int, double>>> &current_PW, vector<vector<pair<int, double>>> &current_PT,
                                                 vector<double> &current_Group_workerAD, vector<double> current_detour_distance[],
                                                 vector<vector<double>> &current_Group_worker_subTrajectoryDis,
                                                 vector<int> current_CT_Worker[], double MaxDistanceTask[], double *current_task_NeedTime,
                                                 vector<int> current_poi[], double nowtime)
{
                                                    ...
                                                    vector<vector<double>> &global_Worker_subTrajectoryDis)
{
    ...
    for (int i = 0; i < tasks_size; i++)
    {
        ...
        if ((sign - temp == Tmax) || (task.startTime >= current_window_endTime))
        {
            ...
            groupWork_according_TaskGroup(workers, Sumdis, current_Group_worker, current_Group_workerSumdis,
                                          current_window_endTime, current_workID, current_Group_workerAD,
                                          global_Worker_subTrajectoryDis, current_Group_worker_subTrajectoryDis);

            determine_Window_Task_Timeout(current_taskGroup, current_window_endTime);

            match_WorkerTask_TPPG_DAG(current_taskGroup, current_Group_worker, current_window_endTime, current_Group_workerAD,
                                      current_Group_worker_subTrajectoryDis);
    ...
    if (current_taskGroup.size() > 0)
    {
        ...
        groupWork_according_TaskGroup(workers, Sumdis, current_Group_worker, current_Group_workerSumdis, current_window_endTime,
                                      current_workID, current_Group_workerAD, global_Worker_subTrajectoryDis, current_Group_worker_subTrajectoryDis);
                                                  ...
                                                  vector<vector<double>> &current_Group_worker_subTrajectoryDis)
{
    ...
    int worker_start_number = current_Group_worker_AW.size();
    ...
    for (int i = 0; i < current_Number_Task; i++)
    {
        ...
        if (current_Group_worker_AW.size() != 0)
        {
            ...
            workerid = FindPreferedWorkerNew_TPPG_DAG(worker_start_number, i, current_Group_worker_AW, current_Group_workerAD, current_detour_distance,
                                                      current_Group_worker_subTrajectoryDis, current_CT_Worker, current_MaxDistanceTask,
                                                      &current_task_NeedTime, current_taskGroup, current_workerGroup, poi, current_window_endTime);
            if (workerid != -1)
            {
                current_workerGroup[workerid].sign = false;
                current_taskGroup[i].sign = false;
                ...
                global_workers[current_workerGroup[workerid].Original_Local].ADdis = current_Group_workerAD[workerid];
                UpdateTaskDeadline(current_taskGroup, workerid, i, current_detour_distance, current_CT_Worker, poi, current_MaxDistanceTask,
                                   current_task_NeedTime);
                                                      ...
                                                      vector<int> poi[], double nowtime)
{
    ...
    int worker_AW_start = current_Group_worker_AW.size();
    for (int j = 0; j < worker_AW_start; j++)
    {
        int workerid = current_Group_worker_AW[j];
        if (current_taskGroup[taskid].task.Minscore <= current_workerGroup[workerid].worker.score)
        {
            ...
            if (time != 0)
            {
                double q = (CT_Worker[workerid].size()) / Capacity;
                double AD = 1 - current_Group_workerAD[workerid] /
                                    ((current_workerGroup[workerid].worker.endTime - current_workerGroup[workerid].worker.startTime) * speed);
                double RC = 1 - (time / worker_start_number);
                preference = preference * q * AD * RC;
            }
            ...
            if (preference > maxpre)
            {
                ...
                if (current_workerGroup[workerid].worker.range >= dist)
                {
                    ...
                    if (current_taskGroup[taskid].task.Reward - (2 * dist * c) > 0)
                    {
                        if (CurrentTask_Satisfy(current_taskGroup, workerid, taskid, current_Group_workerAD, current_detour_distance,
                                                current_Group_worker_subTrajectoryDis, CT_Worker, poi, MaxDistanceTask, current_task_NeedTime, nowtime))
...

{
    ...
    for (int i = 0; i < tasks.size(); i++)
    {
        ...
        if ((sign - temp == Tmax) || (task.startTime >= current_window_endTime))
        {
            ...
            match_WorkerTask_TPPG(current_taskGroup, current_Group_worker, current_window_endTime, current_Group_workerAD,
                                  current_Group_worker_subTrajectoryDis);
                                              ...
                                              vector<vector<double>> &current_Group_worker_subTrajectoryDis)
{
    ...
    for (int i = 0; i < current_Number_Task; i++)
    {
        ...
        if (current_Group_worker_AW.size() != 0)
        {
            ...
            workerid = FindPreferedWorkerNew_TPPG(i, current_Group_worker_AW, current_Group_workerAD, current_detour_distance,
                                                  current_Group_worker_subTrajectoryDis, current_CT_Worker, current_MaxDistanceTask,
                                                  &current_task_NeedTime, current_taskGroup, current_workerGroup, poi, current_window_endTime);
            if (workerid != -1)
            {
