...
int main(int argc, char const *argv[])
{

    {
        ...
        int nCapacity[4] = {1, 3, 5, 7};
        int nTmax[8] = {20, 40, 60, 80, 100, 120, 140, 160};
        int nWmax[8] = {2, 4, 6, 8, 10, 12, 14};
            ...
            for (int algorithmOption = 0; algorithmOption < 4; algorithmOption++)
            {
                for (int test_taskNum = 1; test_taskNum <= 1; test_taskNum++)
                {
                    for (int test_workerNum = 1; test_workerNum <= 1; test_workerNum++)
                    {
                        ...
                        {
                            for (int test_Capacity = 0; test_Capacity <= 3; test_Capacity++)
                            {
                                for (int test_Tmax = 1; test_Tmax <= 1; test_Tmax++)
                                {
                                    for (int test_Wmax = 1; test_Wmax <= 1; test_Wmax++)
                                    {
                                        ...
                                        Basic_information info(c, speed, Number_Task * test_taskNum, Number_Worker * test_workerNum,
                                                               nCapacity[test_Capacity], Wmax * test_Wmax, Tmax * test_Tmax, sati_state, dataOption);

                                        vector<vector<double>> Worker_subTrajectoryDis(info.Number_Worker);
                                        vector<double> Sumdis(info.Number_Worker, 0);
                                        bool dataset_state = optionDataset(distributionOption, dataOption, info, Worker_subTrajectoryDis, Sumdis, workEndtimeX, taskEndtimeX, rangeX, scoreX);
                                        cout << "获取数据结束" << endl;
                                        ...
                                        for (int repeat_Num = 0; repeat_Num < 1; repeat_Num++)
                                        {
                                            ...
                                            if (dataset_state)
                                            {
                                                if (algorithmOption == 0)
                                                {
                                                    ...
                                                    info.Grouping_Framework_Greedy(info.global_tasks, info.global_workers, info.Wmax,
                                                                                   info.Tmax, info.global_Sumdis, info.global_Worker_subTrajectoryDis);
                                                    auto end = std::chrono::high_resolution_clock::now();
                                                    auto duration = std::chrono::duration_cast<std::chrono::milliseconds>(end - start);
                                                    info.printf_Satisfaction_Results_num(alg_name, duration.count(), test_rangeNum, test_taskNum, test_workerNum);
                                                }
                                        ...
                                        cout << alg_name + "\t 容量：" << info.Capacity << endl;
