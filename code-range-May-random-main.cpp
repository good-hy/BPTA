...
int main(int argc, char const *argv[])
{

    {
        ...
        int ntask[6];
        int nworker[5];
        int nrange[6] = {500, 1000, 1500, 2000, 2500, 3000};
...

        {
            for (int algorithmOption = 0; algorithmOption < 4; algorithmOption++)
            {
                for (int test_taskNum = 1; test_taskNum <= 1; test_taskNum++)
                {
                    ...
                    {
                        for (int test_rangeNum = 0; test_rangeNum <= 5; test_rangeNum++)
                        {
                            for (int test_Capacity = 1; test_Capacity <= 1; test_Capacity++)
                            {
                                for (int test_Tmax = 1; test_Tmax <= 1; test_Tmax++)
                                {
                                    for (int test_Wmax = 1; test_Wmax <= 1; test_Wmax++)
                                    {
                                        ...
                                        Basic_information info(c, speed, Number_Task * test_taskNum, Number_Worker * test_workerNum, Capacity * test_Capacity, Wmax * test_Wmax, Tmax * test_Tmax, sati_state, dataOption);
                                        ...
                                        for (int repeat_Num = 0; repeat_Num < 1; repeat_Num++)
                                        {
                                            ...
                                            if (dataset_state)
                                            {
                                                if (algorithmOption == 0)
                                                {
                                                    ...
                                                    info.printf_Satisfaction_Results_num(alg_name, duration.count(), nrange[test_rangeNum], test_taskNum, test_workerNum);
                                                }
                                        ...
                                        cout << alg_name << "\t 范围：" << nrange[test_rangeNum] << endl;

                                        cout << "\t任务数：" << info.global_tasks.size() << "  工人数：" << info.global_workers.size() << "  工人容量:" << info.Capacity
                                             << "  工人成本/单位" << info.c << "  移动速度:" << info.speed << "  时间窗口:" << info.Wmax << "  任务窗口:" << info.Tmax << "  半径:" << nrange[test_rangeNum] << endl;
