...
int main(int argc, char const *argv[])
{

    {
        ...
        if (dataOption == 1)
        {
            ...
            Number_Task = 2000;
            Number_Worker = 500;
            int ntask1[5] = {1000, 1500, 2000, 2500, 3000};
        ...
        else if (dataOption == 2)
        {
            ...
            Number_Task = 2000;
            Number_Worker = 100;
            int ntask1[5] = {150, 250, 350, 450, 550};
        ...
        else if (dataOption == 3)
        {
            Number_Task = 10000;
            Number_Worker = 3000;
            int ntask1[5] = {1000, 5000, 10000, 15000, 20000};
...

        {
            ...
            {
                for (int test_taskNum = 0; test_taskNum <= 4; test_taskNum++)
                {
                    for (int test_workerNum = 1; test_workerNum <= 1; test_workerNum++)
                    {
                        for (int test_rangeNum = 1; test_rangeNum <= 1; test_rangeNum++)
                        {
                            for (int test_Capacity = 1; test_Capacity <= 1; test_Capacity++)
                            {
                                ...
                                {
                                    for (int test_Wmax = 1; test_Wmax <= 1; test_Wmax++)
                                    {
                                        ...
                                        Basic_information info(c, speed, ntask[test_taskNum], Number_Worker * test_workerNum, Capacity * test_Capacity, Wmax * test_Wmax, Tmax * test_Tmax, sati_state, dataOption);

                                        vector<vector<double>> Worker_subTrajectoryDis(info.Number_Worker);
                                        vector<double> Sumdis(info.Number_Worker, 0);
                                        bool dataset_state = optionDataset(distributionOption, dataOption, info, Worker_subTrajectoryDis, Sumdis, workEndtimeX, taskEndtimeX, rangeX * test_rangeNum, scoreX);
                                        cout << "获取数据结束" << endl;
                                        ...
                                        for (int repeat_Num = 0; repeat_Num < 1; repeat_Num++)
                                        {
                                            ...
                                            if (dataset_state)
                                            {
                                                if (algorithmOption == 0)
                                                {
                                                    ...
                                                    info.printf_Satisfaction_Results_num(alg_name, duration.count(), test_rangeNum, ntask[test_taskNum], test_workerNum);
                                                }
                                                ...
                                                else if (algorithmOption == 11)
                                                {
                                                    ...
                                                    info.begin_Algorithm(alg_name);
                                                    auto start = std::chrono::high_resolution_clock::now();
                                                    info.Grouping_Framework_TPPG_DAG(info.global_tasks, info.global_workers, info.Wmax, info.Tmax, info.global_Sumdis, info.global_Worker_subTrajectoryDis);
                                                    auto end = std::chrono::high_resolution_clock::now();
                                                    auto duration = std::chrono::duration_cast<std::chrono::milliseconds>(end - start);
                                                    info.printf_Satisfaction_Results_num(alg_name, duration.count(), test_rangeNum, Number_Task * test_taskNum, test_workerNum);
                                                }
                                        ...
                                        cout << alg_name + "\t 任务：" << info.global_tasks.size() << endl;
