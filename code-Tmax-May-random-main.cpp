...
int main(int argc, char const *argv[])
{

    {
        ...
        else if (dataOption == 3)
        {
            ...
            int nTmax[8] = {25, 100, 250, 500, 1000, 2000, 4000, 8000};
            std::copy(std::begin(ntask1), std::end(ntask1), std::begin(ntask));
            std::copy(std::begin(nworker1), std::end(nworker1), std::begin(nworker));
        }
                ...
                for (int test_taskNum = 1; test_taskNum <= 1; test_taskNum++)
                {
                    for (int test_workerNum = 1; test_workerNum <= 1; test_workerNum++)
                    {
                        for (int test_rangeNum = 1; test_rangeNum <= 1; test_rangeNum++)
                        {
                            ...
                            {
                                for (int test_Tmax = 0; test_Tmax <= 3; test_Tmax++)
                                {
                                    for (int test_Wmax = 1; test_Wmax <= 1; test_Wmax++)
                                    {
                                        ...
                                        Basic_information info(c, speed, Number_Task * test_taskNum, Number_Worker * test_workerNum, Capacity * test_Capacity, Wmax * test_Wmax, nTmax[test_Tmax], sati_state, dataOption);
                                        ...
                                        cout << alg_name + "\t 窗口中任务大小：" << info.Tmax << endl;
